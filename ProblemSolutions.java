...
public class ProblemSolutions {
 ...
 public static int lastBoulder(int[] boulders) {
        ...
        public static ArrayList<String> showDuplicates(ArrayList<String> input) {
                ...
                //loop throught the array list, create a temp string for the inputs
                for (int i = 0; i < input.size(); i++) {
                        String str = input.get(i);
                        if (seen.contains(str)) {
                                // Add to duplicates if already seen
                                duplicates.add(str);
                        } else {
                                seen.add(str);
                        }
                ...
                //sort list using the collections
                ArrayList<String> duplStrings = new ArrayList<>(duplicates);
                Collections.sort(duplStrings);
                return duplStrings;
        }
        ...
        public static ArrayList<String> pair(int[] input, int k) {
                ...
                Arrays.sort(input); //sort the array first
                ...
                while (low<high) {
                     ...
                     if (sum == k) {
                             pairs.add("(" + input[low] + ", " + input[high] + ")");
                                low++;
                                high--;

                        } else if (sum < k) { //move pointer up
                                low++;
                        } else { //move pointer down
                                high--;
                        }
                }
